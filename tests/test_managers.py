...
def test_registrar_venta_correcta():
    ...
    num_reserva = "" #se genera aleatoriamente al registrarse la venta
    precio_pagado_usd = round(db_manager.consultar_columna_unica("SELECT costo_operativo_usd FROM vuelos WHERE id = %s", (id_vuelo, ))[0] * Decimal("1.3"), 2)
    ...
    consulta_ventas: list[FilaVenta] = db_manager.consultar("SELECT * FROM ventas ORDER BY id DESC LIMIT 1")

    venta = VentaDesdeDB(*consulta_ventas[0])
